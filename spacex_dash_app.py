...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':    
        ...
        fig1 = px.pie(filtered_df, values=values, 
        names=names, 
...
def get_scatter_chart(entered_site,entered_payload_range):
    if entered_site == 'ALL': 
        ...
        low, high = entered_payload_range
        mask = (filtered_df_1['Payload Mass (kg)'] > low) & (filtered_df_1['Payload Mass (kg)'] < high)
        fig2 = px.scatter(
        filtered_df_1[mask], x="Payload Mass (kg)", y="class", 
        color="Booster Version Category", title='Correlation between Payload and Success for for site ' + entered_site)
        return fig2
